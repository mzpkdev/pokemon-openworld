...
def render(manifest: dict[str, Any], values: dict[str, int]) -> str:
    ...
    lines = [
        ...
        f"#define FRLG_TRAINER_REMATCH_BINDING_COUNT {FRLG_BINDING_COUNT}",
        "",
    ...
    lines.extend(
        [
            "};",
            "",
            "static const struct TrainerRematchBinding sTrainerRematchBindings_FRLG[FRLG_TRAINER_REMATCH_BINDING_COUNT] =",
            "{",
        ]
    )
    for index, row in enumerate(rows):
    ...
    for symbol, value in NONE_BINDINGS.items():
        if value >= FRLG_BINDING_COUNT:
            continue
        lines.append(
