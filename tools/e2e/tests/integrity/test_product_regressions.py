...
REGIONAL_STORY_MIGRATION_MARKER = bytes((0x53, 3))
...
def test_existing_hoenn_save_continues(game_from_hoenn_save):
    ...
    assert original.active_slot.trainer_defeated_bitmap == bytes(103)
    ...
    assert rewritten.active_slot.trainer_defeated_bitmap == bytes(103)
...
def test_populated_historical_save_preserves_reviewed_state(
    ...
    assert image.active_slot.trainer_defeated_bitmap == bytes(103)
