...
result = subprocess.run(
    ["arm-none-eabi-objdump", "-t", args.filename], stdout=subprocess.PIPE
)
...
for line in result.stdout.decode("utf-8").splitlines():
    ...
    if len(columns) in {5, 6} and "*ABS*" not in line:
        ...
        symbols.append(
            Symbol(
                name=name, address_start=address_start, address_end=address_start + size
            )
        )
...
for symbol in symbols:
    if symbol.address_start <= args.address < symbol.address_end:
        print(
            "{} (offset {})".format(
                symbol.name, hex(args.address - symbol.address_start)
            )
        )
        found = True
    elif not found:
        if args.address > symbol.address_end and (
            not closest_before or symbol.address_end > closest_before.address_end
        ):
            closest_before = symbol
        if args.address < symbol.address_start and (
            not closest_after or symbol.address_start < closest_after.address_start
        ):
    ...
    if closest_before or closest_after:
        ...
        if closest_before:
            print(
                "{} at address [{}..{})".format(
                    closest_before.name,
                    hex(closest_before.address_start),
                    hex(closest_before.address_end),
                )
            )
        if closest_after:
            print(
                "{} at address [{}..{})".format(
                    closest_after.name,
                    hex(closest_after.address_start),
                    hex(closest_after.address_end),
                )
            )
    quit(1)
