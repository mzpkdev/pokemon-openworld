...
class Config:
    def __init__(self, config_file_name, rtc_constants_file_name, encounters_json_data):
        ...
        if self.times_of_day is None:
            raise Exception(
                f"Failed to parse 'enum TimeOfDay' in '{rtc_constants_file_name}'"
            )

        self.ParseMonTypes(encounters_json_data)
        if self.mon_types is None:
    ...
    def ParseTimeEnum(self, rtc_constants_file_name):
        with open(rtc_constants_file_name, "r") as rtc_constants_file:
            DEFAULT_TIME_PAT = re.compile(
                r"enum\s+TimeOfDay\s*\{(?P<rtc_val>[\s*\w+,\=\d*]+)\s*\}\s*\;"
            )
            ...
            if m:
                ...
                self.times_of_day = {}
                for value in values:
                    self.times_of_day[value] = (
                        value.title().replace("Time_", "").replace("_", "")
                    )
...
class WildEncounterAssembler:
            ...
            if "fields" in wild_encounter_group:
                ...
                for field in fields:
                    ...
                    for idx, rate in enumerate(encounter_rates):
                        macro_name = (
                            macro_base + group_name_mapping[idx] + "_SLOT_" + str(idx)
                        )
                        macro_value = str(rate)
                        if previous_group == group_name_mapping[idx]:
                            macro_value = (
                                "(" + previous_macro + " + " + macro_value + ")"
                            )
                        elif idx > 0:
                            macro_total_name = (
                                macro_base + group_name_mapping[idx - 1] + "_TOTAL"
                            )
                            self.WriteMacro(
                                macro_total_name, "(" + previous_macro + ")"
                            )
                        ...
                        if idx == len(encounter_rates) - 1:
                            macro_total_name = (
                                macro_base + group_name_mapping[idx] + "_TOTAL"
                            )
                            self.WriteMacro(
                                macro_total_name, "(" + previous_macro + ")"
                            )
    ...
    def WriteMonInfos(self, name, mons, encounter_rate):
        ...
        self.WriteLine()
        self.WriteLine(
            f"const struct WildPokemonInfo {info_name} = {{ {encounter_rate}, {name} }};"
        )
        self.WriteLine()
        ...
        for shared_label in headers["data"]:
            ...
            for time in self.config.times_of_day:
                if (
                    ...
                    and time != self.config.time_fallback
                ):
                    continue
            ...
            for map_encounters in encounters:
                ...
                for time_ident in self.config.times_of_day:
                    if self.config.times_of_day[time_ident] in base_label:
                        time = time_ident
                        shared_label = shared_label.replace(
                            "_" + self.config.times_of_day[time_ident], ""
                        )
                ...
                for mon_type in self.config.mon_types:
                    ...
                    mon_array_name = (
                        base_label + "_" + mon_type.title().replace("_", "")
                    )
                    self.WriteMonInfos(mon_array_name, mons, encounter_rate)
                    headers["data"][shared_label][time][mon_type] = (
                        mon_array_name + "Info"
                    )
                self.WriteLine("#endif")
...
def ConvertToHeaderFile(json_data):
    with open("src/data/wild_encounters.h", "w") as output_file:
        config = Config(
            "include/config/overworld.h", "include/constants/rtc.h", json_data
        )
