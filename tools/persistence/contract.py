...
SCHEMA_VERSION = 1
CONTRACT_METADATA_KEYS = frozenset(("baselineCommit", "purposeBudgets", "purposeAbiEvidence"))
ABI_PURPOSES = ("normal", "debug", "release", "test-runner", "headless-test")
ROOT_TYPES = (
    "SaveBlock1", "SaveBlock2", "SaveBlock3", "PokemonStorage", "BoxPokemon",
    "Pokemon", "PokemonSubstruct0", "PokemonSubstruct1", "PokemonSubstruct2",
    "PokemonSubstruct3", "DayCare", "BattleFrontier", "EmeraldBattleTowerRecord",
    "RSBattleTowerRecord", "RecordedBattleSave", "TrainerHillSave",
    "TrainerHillChallenge", "HallofFameTeam", "GabbyAndTyData", "SaveSector",
    "PlayerRecordEmerald", "PlayerRecordRS",
)
...
def compare(expected: Any, actual: Any, path: str = "$") -> None:
    ...
    if isinstance(expected, dict):
        if expected.keys() != actual.keys():
            ...
            raise ContractError(f"{path}: keys changed (missing={missing}, extra={extra})")
        for key in expected:
            compare(expected[key], actual[key], f"{path}.{key}")
    elif isinstance(expected, list):
        if len(expected) != len(actual):
            raise ContractError(f"{path}: length changed ({len(expected)} != {len(actual)})")
...
def _run(args: list[str], cwd: Path, *, input_bytes: bytes | None = None,
    ...
    try:
        ...
        raise ContractError(f"command failed: {' '.join(args)}\n" + "\n".join(tail)) from exc
...
def export_baseline(repo: Path, revision: str, destination: Path) -> str:
    ...
    proc = subprocess.run(["tar", "-xf", "-", "-C", str(destination)], input=archive,
...
def _purpose_defines(purpose: str) -> list[str]:
    ...
    defines = ["-DMODERN=1", f"-DTESTING={int(testing)}", "-DEMERALD=1", "-DALL_REGIONS=1"]
    if purpose == "debug":
...
def _compile_units(tree: Path, output_dir: Path, purpose: str) -> list[Path]:
    ...
    common = [
        "arm-none-eabi-gcc", "-c", "-g3", "-gdwarf-4",
        "-fno-eliminate-unused-debug-types", "-O0", "-mthumb", "-mthumb-interwork",
        "-mabi=apcs-gnu", "-mtune=arm7tdmi", "-march=armv4t", "-std=gnu17",
        *_purpose_defines(purpose),
        "-Wno-pointer-to-int-cast", "-Wno-address-of-packed-member",
        "-iquote", str(generated / "include/constants"),
        "-iquote", str(generated / "src"), "-iquote", str(generated / "include"),
        "-iquote", str(tree / "include"),
        "-include", str(generated / "include/constants/map_groups.h"),
        "-include", str(generated / "include/constants/layouts.h"),
        "-include", str(generated / "include/constants/map_event_ids.h"),
    ]
...
_DIE_RE = re.compile(r"^\s*<(\d+)><([0-9a-f]+)>: Abbrev Number: \d+(?: \((DW_TAG_[^)]+)\))?")
_ATTR_RE = re.compile(r"^\s*<[0-9a-f]+>\s+(DW_AT_[A-Za-z0-9_]+)\s*:\s*(.*)$")
_REF_RE = re.compile(r"<0x([0-9a-f]+)>")


def _attr_text(raw: str) -> str:
    # readelf prefixes indirect strings with metadata ending in ': '.
...
def _parse_dwarf(text: str) -> dict[int, dict[str, Any]]:
    ...
    for line in text.splitlines():
        ...
        if match:
            depth, offset, tag = int(match.group(1)), int(match.group(2), 16), match.group(3)
            stack = stack[:depth]
            if not tag:
                current = None
                continue
...
def _merge_dies(objects: Iterable[Path], tree: Path) -> list[dict[int, dict[str, Any]]]:
    result = []
    for obj in objects:
        dump = _run(["arm-none-eabi-readelf", "--debug-dump=info", "--wide", str(obj)], tree)
...
class DwarfLayouts:
    ...
    def collect(self) -> tuple[dict[str, Any], dict[str, int]]:
        for unit_index, dies in enumerate(self.units):
            for offset, die in dies.items():
                ...
                if die["tag"] in ("DW_TAG_structure_type", "DW_TAG_union_type") and _die_name(die):
                    self._record(unit_index, offset)
        missing = [name for name in ROOT_TYPES if name not in self.structs]
        if missing:
            raise ContractError(f"required ARM DWARF roots missing: {', '.join(missing)}")
        return dict(sorted(self.structs.items())), dict(sorted(self.enums.items()))
    ...
    def _record(self, unit_index: int, offset: int) -> str:
        ...
        if name in self.structs or key in self._building:
            return name
        ...
        for child_offset in die["children"]:
            ...
            if "DW_AT_bit_size" in child["attrs"]:
                ...
                if bit_offset is not None:
                    ...
                    member["legacyBitOffset"] = _attr_int(child["attrs"]["DW_AT_bit_offset"])
            ref = _type_ref(child)
            member["type"] = self._describe(unit_index, ref) if ref is not None else {"kind": "void"}
        ...
        alignment = _attr_int(explicit) if explicit else self._aggregate_alignment(size, members, member_alignments)
        self.structs[name] = {
    ...
    def _describe(self, unit_index: int, offset: int) -> dict[str, Any]:
        ...
        if tag in ("DW_TAG_typedef", "DW_TAG_const_type", "DW_TAG_volatile_type", "DW_TAG_restrict_type"):
            ref = _type_ref(die)
            value = self._describe(unit_index, ref) if ref is not None else {"kind": "void"}
        ...
        if tag in ("DW_TAG_structure_type", "DW_TAG_union_type"):
            return {"kind": "struct" if tag == "DW_TAG_structure_type" else "union",
                    "name": self._record(unit_index, offset)}
        if tag == "DW_TAG_array_type":
            ref = _type_ref(die)
            element = self._describe(unit_index, ref) if ref is not None else {"kind": "void"}
            dimensions = []
            for child_offset in die["children"]:
                child = dies[child_offset]
                ...
                if "DW_AT_count" in child["attrs"]:
                    ...
                    dimensions.append(_attr_int(child["attrs"]["DW_AT_upper_bound"]) + 1)
                else:
                    raise ContractError("array has no target-readable bound")
            return {"kind": "array", "dimensions": dimensions, "element": element}
        if tag == "DW_TAG_pointer_type":
            ref = _type_ref(die)
            return {"kind": "pointer", "size": _attr_int(die["attrs"].get("DW_AT_byte_size", "4")),
                    "target": self._describe(unit_index, ref) if ref is not None else {"kind": "void"}}
        if tag == "DW_TAG_enumeration_type":
            return {"kind": "enum", "name": _die_name(die) or "<anonymous>",
                    "size": _attr_int(die["attrs"].get("DW_AT_byte_size", "4"))}
        if tag == "DW_TAG_base_type":
            return {"kind": "base", "name": _die_name(die) or "<anonymous>",
                    "size": _attr_int(die["attrs"].get("DW_AT_byte_size", "0")),
                    "encoding": _attr_text(die["attrs"].get("DW_AT_encoding", "unknown"))}
        if tag == "DW_TAG_subroutine_type":
            return {"kind": "function"}
        return {"kind": tag.removeprefix("DW_TAG_"), "name": _die_name(die) or "<anonymous>"}
    ...
    @staticmethod
    def _aggregate_alignment(size: int, members: list[dict[str, Any]], aligns: list[int]) -> int:
...
_ALLOWED_BINOPS = {ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul,
                   ...
                   ast.RShift: operator.rshift, ast.BitOr: operator.or_, ast.BitAnd: operator.and_, ast.BitXor: operator.xor}
...
def _eval_expr(expr: str, values: dict[str, int]) -> int:
    ...
    def visit(item: ast.AST) -> int:
        if isinstance(item, ast.Constant) and isinstance(item.value, int): return item.value
        if isinstance(item, ast.Name) and item.id in values: return values[item.id]
...
def _extract_constants(tree: Path, generated: Path, enums: dict[str, int], purpose: str) -> dict[str, int]:
    args = ["arm-none-eabi-gcc", "-dM", "-E", "-x", "c", "-", *_purpose_defines(purpose),
            "-iquote", str(generated / "include/constants"),
            "-iquote", str(generated / "include"), "-iquote", str(tree / "include"),
            "-include", "global.h", "-include", "save.h", "-include", "hall_of_fame.h",
    ...
    for _ in range(20):
        ...
        for name, expr in expressions.items():
            ...
            try: values[name] = _eval_expr(expr, values); changed = True
...
def _bindings(values: dict[str, int]) -> dict[str, list[dict[str, Any]]]:
    domains = {
        "trainerIds": ("TRAINER_",), "flags": ("FLAG_",), "vars": ("VAR_",),
        ...
        "savedLocations": ("MAPSEC_",), "metLocations": ("METLOC_",),
        "gameStats": ("GAME_STAT_",),
    }
    result = {}
    for domain, prefixes in domains.items():
        entries = [{"symbol": name, "value": value} for name, value in values.items()
                   if name.startswith(prefixes)]
        result[domain] = sorted(entries, key=lambda item: (item["value"], item["symbol"]))
    return result


def _physical(values: dict[str, int], structs: dict[str, Any]) -> dict[str, Any]:
    names = ("SECTOR_DATA_SIZE", "SAVE_BLOCK_3_CHUNK_SIZE", "SECTOR_FOOTER_SIZE", "SECTOR_SIZE",
             "NUM_SAVE_SLOTS", "SECTOR_SIGNATURE", "SPECIAL_SECTOR_SENTINEL",
             "SECTOR_ID_SAVEBLOCK2", "SECTOR_ID_SAVEBLOCK1_START", "SECTOR_ID_SAVEBLOCK1_END",
             "SECTOR_ID_PKMN_STORAGE_START", "SECTOR_ID_PKMN_STORAGE_END", "NUM_SECTORS_PER_SLOT",
             "SECTOR_ID_HOF_1", "SECTOR_ID_HOF_2", "SECTOR_ID_TRAINER_HILL",
             "SECTOR_ID_RECORDED_BATTLE", "SECTORS_COUNT")
    missing = [name for name in names if name not in values]
    if missing: raise ContractError(f"physical constants missing: {', '.join(missing)}")
    ...
    roots = (("SaveBlock2", values["SECTOR_ID_SAVEBLOCK2"]),
    ...
    for structure, first_sector in roots:
        ...
        while remaining:
            ...
            slot_layout.append({"sectorId": sector_id, "payload": {"structure": structure,
                                "offset": chunk * values["SECTOR_DATA_SIZE"], "size": size},
                                "saveBlock3Chunk": {"offset": sector_id * values["SAVE_BLOCK_3_CHUNK_SIZE"],
                                "size": min(max(structs["SaveBlock3"]["size"] - sector_id * values["SAVE_BLOCK_3_CHUNK_SIZE"], 0),
                                            values["SAVE_BLOCK_3_CHUNK_SIZE"])}})
            remaining -= size
            chunk += 1
    return {"constants": constants, "slotLayout": slot_layout,
            "saveBlock3Stream": {"sectorOffset": values["SECTOR_DATA_SIZE"],
                "chunkSize": values["SAVE_BLOCK_3_CHUNK_SIZE"],
                "chunkCount": values["NUM_SECTORS_PER_SLOT"],
                "capacity": values["SAVE_BLOCK_3_CHUNK_SIZE"] * values["NUM_SECTORS_PER_SLOT"],
                "serializedSize": structs["SaveBlock3"]["size"]},
            "slotSectorIds": {
        "saveBlock2": [values["SECTOR_ID_SAVEBLOCK2"]],
        "saveBlock1": list(range(values["SECTOR_ID_SAVEBLOCK1_START"], values["SECTOR_ID_SAVEBLOCK1_END"] + 1)),
        "pokemonStorage": list(range(values["SECTOR_ID_PKMN_STORAGE_START"], values["SECTOR_ID_PKMN_STORAGE_END"] + 1)),
...
def _checksums(structs: dict[str, Any]) -> dict[str, Any]:
    def member_offset(type_name: str, member: str) -> int:
        for item in structs[type_name]["members"]:
            if item["name"] == member: return item["offset"]
        raise ContractError(f"{type_name}.{member} missing from DWARF")
    return {
        "mainSector": {"algorithm": "sum-le32-fold16", "coverage": {"ranges": "physical.slotLayout[*].payload",
                       "wordSize": 4, "trailingPartialWord": "excluded"}},
        "boxPokemon": {"algorithm": "sum-le16-mod65536", "coverage": {"member": "BoxPokemon.secure", "offset": member_offset("BoxPokemon", "secure"), "size": 48},
                       "encryption": {"algorithm": "xor-le32", "key": "personality ^ otId"}},
        "emeraldRecordMixing": {"algorithm": "sum-le32-mod2^32", "coverage": {"type": "EmeraldBattleTowerRecord", "start": 0, "end": member_offset("EmeraldBattleTowerRecord", "checksum")}},
        "rsRecordMixing": {"algorithm": "sum-le32-mod2^32", "coverage": {"type": "RSBattleTowerRecord", "start": 0, "end": member_offset("RSBattleTowerRecord", "checksum")}},
        "trainerHill": {"algorithm": "sum-u8-mod2^32", "coverage": {"type": "TrainerHillChallenge",
                        "start": structs["TrainerHillChallenge"]["size"],
                        "end": "start + numFloors * sizeof(TrainerHillFloor)"}},
        "recordedBattle": {"algorithm": "sum-u8-mod2^32", "coverage": {"type": "RecordedBattleSave", "start": 0, "end": member_offset("RecordedBattleSave", "checksum")}},
        "specialSectors": {"algorithm": "raw-copy", "coverage": {"start": 4, "end": structs["SaveSector"]["size"]},
                           "sentinel": {"offset": 0, "size": 4, "identifier": "SPECIAL_SECTOR_SENTINEL"}},
    }
...
def _source_evidence(tree: Path) -> dict[str, Any]:
    sources = {
        "src/save.c": ("CalculateChecksum", "TryReadSpecialSaveSector", "TryWriteSpecialSaveSector",
                       "HandleWriteSector", "UpdateSaveAddresses", "SaveBlock3Size",
                       "CopyToSaveBlock3", "CopyFromSaveBlock3"),
        "src/pokemon.c": ("CalculateBoxMonChecksum", "CalculateBoxMonChecksumDecrypt", "CalculateBoxMonChecksumReencrypt"),
        "src/battle_tower.c": ("CalcEmeraldBattleTowerChecksum", "CalcRubyBattleTowerChecksum"),
    ...
    if not layout_match:
        raise ContractError("serialized mechanics initializer is missing: sSaveSlotLayout")
    layout = layout_match.group(0)
    evidence["sSaveSlotLayout"] = {"source": "src/save.c",
    ...
    required_fragments = {
        ...
        "CalcEmeraldBattleTowerChecksum": ("sizeof(struct EmeraldBattleTowerRecord) - 4",),
        ...
        "TryWriteTrainerHill_Internal": ("challenge->floors", "sizeof(struct TrainerHillFloor)", "CalcByteArraySum"),
        "TryReadSpecialSaveSector": ("SPECIAL_SECTOR_SENTINEL", "SECTOR_COUNTER_OFFSET - 1"),
        "TryWriteSpecialSaveSector": ("SPECIAL_SECTOR_SENTINEL", "SECTOR_COUNTER_OFFSET - 1"),
        "HandleWriteSector": ("locations[sectorId].data", "locations[sectorId].size",
                              "CopyFromSaveBlock3", "CalculateChecksum"),
        "UpdateSaveAddresses": ("sSaveSlotLayout[i].offset", "sSaveSlotLayout[i].size",
                                "gSaveBlock2Ptr", "gSaveBlock1Ptr", "gPokemonStoragePtr"),
    ...
    for name, fragments in required_fragments.items():
        ...
        if missing:
            raise ContractError(f"{name}: serialized mechanics changed near {missing[0]!r}")
...
def _walk_layout_graph(structs: dict[str, Any], roots: Iterable[str]) -> set[str]:
    ...
    def walk_desc(desc: dict[str, Any], path: str) -> None:
        ...
        if kind in ("struct", "union"):
            ...
            if not desc.get("dimensions") or any(not isinstance(v, int) or v < 0 for v in desc["dimensions"]):
    ...
    def walk_struct(name: str) -> None:
        ...
        reached.add(name)
        layout = structs[name]
        if not isinstance(layout.get("size"), int) or not isinstance(layout.get("alignment"), int):
...
def abi_evidence_values(abi: dict[str, Any]) -> list[tuple[str, int]]:
    """Return each unique linked structural ABI fact exactly once.

    The JSON contract remains the descriptive authority.  The ROM table only
    needs target layout facts: each reachable inline type's size/alignment,
    each member offset, bitfield shape, and array cardinality.  Recursively
    embedding type descriptions here would multiply the same fact thousands
    of times and consume ROM without strengthening the proof.
    """
    structs = abi["structs"]
    ...
    def array_cardinality(desc: dict[str, Any]) -> int | None:
        kind = desc.get("kind")
        if kind == "typedef":
            return array_cardinality(desc["target"])
        if kind != "array":
            return None
        cardinality = 1
        for dimension in desc["dimensions"]:
            cardinality *= dimension
        nested = array_cardinality(desc["element"])
        return cardinality * (nested if nested is not None else 1)

    for name in sorted(aliases, key=lambda item: aliases[item]):
        layout = structs[name]
        base = f"$.structs.{aliases[name]}"
        if layout["size"] > 0xFFFFFF or layout["alignment"] > 0xFF:
            raise ContractError(f"{base}: size/alignment cannot be encoded")
        values.append((f"{base}.sizeAlignment",
                       ((layout["size"] << 8) | layout["alignment"]) & 0xFFFFFFFF))
        for index, member in enumerate(layout["members"]):
            member_path = f"{base}.members[{index}]"
            values.append((f"{member_path}.offset", member["offset"] & 0xFFFFFFFF))
            if "bitSize" in member:
                bit_offset = member.get("bitOffset", member.get("legacyBitOffset"))
                if bit_offset is None or bit_offset > 0xFFFFFF or member["bitSize"] > 0xFF:
                    raise ContractError(f"{member_path}: bitfield shape cannot be encoded")
                values.append((f"{member_path}.bitOffsetSize",
                               ((bit_offset << 8) | member["bitSize"]) & 0xFFFFFFFF))
            cardinality = array_cardinality(member["type"])
            if cardinality is not None:
                values.append((f"{member_path}.arrayCardinality", cardinality & 0xFFFFFFFF))
    return values
...
def abi_evidence_values_for_purpose(contract: dict[str, Any], purpose: str) -> list[tuple[str, int]]:
    if purpose not in ABI_PURPOSES:
        raise ContractError(f"unknown ABI purpose: {purpose}")
...
def measure_tree(tree: Path, purpose: str = "normal") -> dict[str, Any]:
    ...
    physical_names = ("sSaveSlotLayout", "HandleWriteSector", "UpdateSaveAddresses",
                      "SaveBlock3Size", "CopyToSaveBlock3", "CopyFromSaveBlock3")
    physical["sourceEvidence"] = {name: checksums["sourceEvidence"][name] for name in physical_names}
    return {"schemaVersion": SCHEMA_VERSION, "target": "arm-none-eabi/armv4t/apcs-gnu",
            "structs": layouts, "physical": physical,
            "checksums": checksums, "publishedBindings": _bindings(values)}
...
def seed_from_commit(repo: Path, baseline: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="save-contract-baseline-") as tmp:
        ...
        if not anchor_source.exists(): raise ContractError("task ABI anchor is missing")
        destination = snapshot / "tools/persistence/abi_anchor.c"
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(anchor_source, destination)
        measurements = {purpose: measure_tree(snapshot, purpose) for purpose in ABI_PURPOSES}
        measured = measurements["normal"]
        invariant = {key: value for key, value in measured.items()
                     if key not in ("structs", "publishedBindings")}
        for purpose, purpose_measurement in measurements.items():
            compare(invariant, {key: value for key, value in purpose_measurement.items()
                                if key not in ("structs", "publishedBindings")},
                    f"$.purposeInvariant.{purpose}")
        purpose_evidence = {purpose: _frozen_evidence(value)
                            for purpose, value in measurements.items()}
    return {"baselineCommit": sha, **measured, "purposeAbiEvidence": purpose_evidence}


def validate_contract(contract: dict[str, Any]) -> None:
    if contract.get("schemaVersion") != SCHEMA_VERSION: raise ContractError("$.schemaVersion: unsupported")
    ...
    if not contract.get("structs"): raise ContractError("$.structs: empty")
    if not contract.get("publishedBindings"): raise ContractError("$.publishedBindings: empty")
    ...
    for name in ROOT_TYPES:
        if name not in contract["structs"]: raise ContractError(f"$.structs.{name}: missing")
    ...
    compare(abi_evidence_values(contract), abi_evidence_values_for_purpose(contract, "normal"),
    ...
    for domain, entries in contract["publishedBindings"].items():
        if not entries: raise ContractError(f"$.publishedBindings.{domain}: empty")
        symbols = [entry["symbol"] for entry in entries]
        if len(symbols) != len(set(symbols)): raise ContractError(f"$.publishedBindings.{domain}: duplicate symbol")
    _verify_checksum_vectors()


def validate_abi(contract: dict[str, Any], actual: dict[str, Any], purpose: str) -> None:
    validate_contract(contract)
    projected = {key: value for key, value in contract.items() if key not in CONTRACT_METADATA_KEYS}
    actual_projected = dict(actual)
    if purpose != "normal":
        projected.pop("structs")
        actual_projected.pop("structs")
        projected.pop("publishedBindings")
        actual_projected.pop("publishedBindings")
    compare(projected, actual_projected)
    compare(abi_evidence_values_for_purpose(contract, purpose), abi_evidence_values(actual),
            f"$.purposeAbiEvidence.{purpose}")


def _validate_purpose_budgets(value: Any) -> None:
    path = "$.purposeBudgets"
    if not isinstance(value, dict) or set(value) != {"schemaVersion", "limits", "baselines"}:
    ...
    for name, record in baselines.items():
        ...
        if not isinstance(record, dict) or set(record) != {"artifact", "romBytes", "ewramBytes", "iwramBytes"}:
        ...
        for field in ("romBytes", "ewramBytes", "iwramBytes"):
            if not isinstance(record[field], int) or isinstance(record[field], bool) or record[field] < 0:
                raise ContractError(f"{record_path}.{field}: expected nonnegative integer")


def _verify_checksum_vectors() -> None:
    data = bytes(range(48))
    words = [int.from_bytes(data[index:index + 4], "little") for index in range(0, 16, 4)]
    total = sum(words)
    if ((total >> 16) + total) & 0xFFFF != 0x4038:
        raise ContractError("checksum mechanics: main-sector fixed vector failed")
    halfwords = [int.from_bytes(data[index:index + 2], "little") for index in range(0, 48, 2)]
...
PURPOSE_ARTIFACTS = {
    ...
    "debug": ("pokemon-openworld-debug.gba", "pokemon-openworld-debug.elf", ["DEBUG=1"]),
    "release": ("pokemon-openworld-release.gba", "pokemon-openworld-release.elf", ["RELEASE=1"]),
    "test-runner": ("pokemon-openworld-test.elf", "pokemon-openworld-test.elf", []),
    "headless-test": ("pokemon-openworld-test-headless.elf", "pokemon-openworld-test-headless.elf", []),
}
...
def _parse_elf_sections(text: str) -> list[dict[str, Any]]:
    pattern = re.compile(
        ...
        r"(?P<size>[0-9a-fA-F]+)\s+\S+\s+(?P<flags>\S*)", re.MULTILINE)
    sections = [{"name": m.group("name"), "type": m.group("type"),
                 "address": int(m.group("address"), 16), "size": int(m.group("size"), 16),
                 "flags": m.group("flags")} for m in pattern.finditer(text)]
    if not sections:
...
def _measure_elf_capacity(tree: Path, elf: Path) -> dict[str, int]:
    ...
    def range_bytes(origin: int, limit: int, *, loadable: bool = False) -> int:
        candidates = [item for item in sections if "A" in item["flags"]
        ...
        return end - origin
    return {"romBytes": range_bytes(0x08000000, 33554432, loadable=True),
            "ewramBytes": range_bytes(0x02000000, 262144),
            "iwramBytes": range_bytes(0x03000000, 32768)}


def seed_budgets(repo: Path, baseline: str, *, rom_max: int, ewram_max: int,
                 iwram_max: int, release_headroom: int) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="save-budget-baseline-") as tmp:
        tree = Path(tmp)
        sha = export_baseline(repo, baseline, tree)
        if sha != baseline:
            raise ContractError(f"budget baseline resolved to {sha}, expected {baseline}")
        ...
        for purpose, (artifact, elf_name, flags) in PURPOSE_ARTIFACTS.items():
            if purpose == "headless-test":
                ...
                if not source.is_file():
                    raise ContractError("baseline test-runner ELF was not built before headless copy")
                shutil.copyfile(source, destination)
                _run([str(tree / "tools/patchelf/patchelf"), str(destination),
                      "gTestRunnerHeadless", r"\x01", "gTestRunnerSkipIsFail", r"\x00"],
                     tree, env=build_env)
            else:
    ...
    return {"schemaVersion": 1,
            "limits": {"romBytes": rom_max, "ewramBytes": ewram_max,
                       "iwramBytes": iwram_max, "releaseHeadroomBytes": release_headroom},
            "baselines": records}
...
def _report_usage(report: dict[str, Any], purpose: str) -> tuple[str, dict[str, int]]:
    ...
    if Path(artifact).name != expected_artifact:
        raise ContractError(f"$.reports.{purpose}.artifact: expected {expected_artifact}")
...
def validate_budgets(contract: dict[str, Any], reports_dir: Path) -> None:
    ...
    if actual_files != expected_files:
        raise ContractError(f"$.reports: expected exactly {sorted(expected_files)}, got {sorted(actual_files)}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    measure.add_argument("--tree", type=Path, required=True); measure.add_argument("--output", type=Path, required=True)
    measure.add_argument("--purpose", choices=ABI_PURPOSES, default="normal")
    seed = sub.add_parser("seed-save-contract")
    seed.add_argument("--baseline", default=BASELINE_SHA); seed.add_argument("--output", type=Path, required=True)
    seed_budget = sub.add_parser("seed-budgets")
    seed_budget.add_argument("--baseline", required=True); seed_budget.add_argument("--contract", type=Path, required=True)
    seed_budget.add_argument("--rom-max", type=int, required=True); seed_budget.add_argument("--ewram-max", type=int, required=True)
    seed_budget.add_argument("--iwram-max", type=int, required=True); seed_budget.add_argument("--release-rom-headroom-min", type=int, required=True)
    validate_budget = sub.add_parser("validate-budgets")
    validate_budget.add_argument("--contract", type=Path, required=True); validate_budget.add_argument("--reports", type=Path, required=True)
    generate_evidence = sub.add_parser("generate-abi-evidence")
    generate_evidence.add_argument("--abi", type=Path, required=True); generate_evidence.add_argument("--output", type=Path, required=True)
    validate = sub.add_parser("validate-contract"); validate.add_argument("--contract", type=Path, required=True)
    check = sub.add_parser("validate"); check.add_argument("--contract", type=Path, required=True); check.add_argument("--abi", type=Path, required=True)
    check.add_argument("--purpose", choices=ABI_PURPOSES, default="normal")
    args = parser.parse_args(argv)
    try:
        if args.command == "measure-abi": write_json(args.output, measure_tree(args.tree, args.purpose)); print(f"PASS: wrote {args.output} for {args.purpose}")
        elif args.command == "seed-save-contract":
            value = seed_from_commit(Path.cwd(), args.baseline)
            if args.output.exists():
                existing = load_json(args.output)
                if "purposeBudgets" in existing: value["purposeBudgets"] = existing["purposeBudgets"]
            validate_contract(value); write_json(args.output, value); print(f"PASS: wrote {args.output} from {value['baselineCommit']}")
            ...
            contract["purposeBudgets"] = seed_budgets(Path.cwd(), args.baseline,
                rom_max=args.rom_max, ewram_max=args.ewram_max, iwram_max=args.iwram_max,
                release_headroom=args.release_rom_headroom_min)
            validate_contract(contract); write_json(args.contract, contract)
            print(f"PASS: seeded five purpose budgets from {args.baseline}")
        elif args.command == "validate-budgets":
            validate_budgets(load_json(args.contract), args.reports); print("PASS: all five purpose budgets")
        elif args.command == "generate-abi-evidence":
            abi = load_json(args.abi); data = render_abi_evidence(abi)
            args.output.parent.mkdir(parents=True, exist_ok=True)
            if not args.output.exists() or args.output.read_bytes() != data: args.output.write_bytes(data)
            print(f"PASS: wrote {len(abi_evidence_values(abi))} linked ABI values to {args.output}")
        elif args.command == "validate-contract": validate_contract(load_json(args.contract)); print(f"PASS: {args.contract}")
        else:
            expected = load_json(args.contract); actual = load_json(args.abi)
