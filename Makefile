...
$(DATA_SRC_SUBDIR)/wild_encounters.h: $(DATA_SRC_SUBDIR)/wild_encounters.json \
                                       ...
                                       $(DATA_SRC_SUBDIR)/wild_encounter_bands.json \
                                       $(DATA_SRC_SUBDIR)/wild_encounter_time_policies.json \
...
RULES_NO_SCAN += _e2e-build-debug-artifacts _e2e-require-artifacts _e2e-skyemu e2e-core e2e-extended e2e-integrity e2e-integrity-full integrity-check integrity-check-rom-purposes save-contract-check start-profile-contract-check build-variant-isolation-check format format-check lint lint-check probe-retrieval-test probe-retrieval-integration-test
...
.PHONY: all rom agbcc modern compare check debug release format format-check lint lint-check probe-retrieval-test probe-retrieval-integration-test
...
probe-retrieval-test: content-port-transaction-check
	PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover \
		-s tools/probe_retrieval/tests -p 'test_*.py' -q

probe-retrieval-integration-test: content-port-transaction-check
	PYTHONDONTWRITEBYTECODE=1 python3 -m unittest \
		tools.probe_retrieval.integration_test -v
...
$(C_BUILDDIR)/data.o: $(DATA_SRC_SUBDIR)/trainers.h $(DATA_SRC_SUBDIR)/trainers_frlg.h
