...
$(DATA_SRC_SUBDIR)/wild_encounters.h: $(DATA_SRC_SUBDIR)/wild_encounters.json \
                                       ...
                                       $(DATA_SRC_SUBDIR)/wild_encounter_scaling.json \
                                       $(DATA_SRC_SUBDIR)/wild_encounter_species.json \
...
RULES_NO_SCAN += wild-encounter-test wild-encounter-balance-audit map-catalog-test map-catalog map-atlas-catalog map-atlas-build map-atlas-test map-atlas-browser-test map-atlas-preview-test
...
.PHONY: map-catalog-test map-catalog map-atlas-catalog map-atlas-build map-atlas-test map-atlas-browser-test map-atlas-preview-test
.PHONY: $(RULES_NO_SCAN)
...
map-atlas-browser-test: map-atlas-build
	npm --prefix "$(MAP_ATLAS_DIR)" run test:browser

map-atlas-preview-test:
	PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover \
		-s tools/map_atlas_preview/tests -p 'test_*.py' -q
