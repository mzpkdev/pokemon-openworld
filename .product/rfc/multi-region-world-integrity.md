...
- Preserve the rendered `CI / Build`, `CI / Format`, `CI / Lint`, `CI / Test`, `CI / E2E (Core)`, `CI / E2E (Extended)`, and `CI / PR Metadata` check identities. `Build` produces the release-authority `pokemon-openworld` artifact containing exactly `pokemon-openworld.gba`, `.map`, `.sym`, and `pokemon-openworld-debug.gba`, plus the separate one-day `pokemon-openworld-debug` artifact containing the debug `.gba` and `.sym` for E2E. The debug `.sym` is not a release asset. `Test` remains the sole CI owner of `make check`.
