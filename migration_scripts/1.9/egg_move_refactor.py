...
for mon in re.findall(r"egg_moves\((.*),", data):
    ...
    data = re.sub(
        r"    egg_moves\(" + mon + r",",
        "static const u16 s%sEggMoveLearnset[] = {" % monname,
        data,
    )

data = re.sub(
    r"\),\n", ",\n        MOVE_UNAVAILABLE,\n};\n", data
)  # add terminator to each old macro
...
for file in glob.glob("./src/data/pokemon/species_info/gen_*_families.h"):
    ...
    for mon in eggMoveSpecies:
        ...
        data = re.sub(
            r"\.teachableLearnset = s" + mon + r"sTeachableLearnset,\n",
        ...
        macrocheck = re.findall(
            r"\.teachableLearnset = s" + mon + r"TeachableLearnset,( *)\\\\", data
        )
        if len(macrocheck) > 0:
            data = re.sub(
                r"\.teachableLearnset = s"
                + mon
                + r"TeachableLearnset,"
                + macrocheck[0]
                + r"\\\\",
                ".teachableLearnset = s%sTeachableLearnset,%s\\\\\n        .eggMoveLearnset = s%sEggMoveLearnset,%s\\\\"
                % (mon, macrocheck[0], mon, " " * (len(macrocheck[0]) + 4)),
                data,
            )
