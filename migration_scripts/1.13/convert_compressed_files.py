...
secondaryTileset_pattern = re.compile(
    r"(.*\"data/tilesets/secondary/.+\.4bpp\.)lz(\".*)"
)
    ...
    with open(fileInput, "r", encoding="UTF-8") as file:
        ...
        while line := file.readline():
            if line.strip() == '#include "decompress.h"':
                ...
                if (
                    allLines[-1].strip() != "case MODE_LZ77:"
                ):  # do not modify DecompressDataWithHeader itself
                    line = (
                        match.group(1)
                        + "DecompressDataWithHeader"
                        + match.group(2)
                        + "ram"
                        + match.group(3)
                        + "\n"
        ...
        if needs_decompress_h and not has_decompress_h:
            ...
            i = 0
            while (
                not allLines[i].startswith('#include "')
                or allLines[i] == '#include "global.h"\n'
            ):
                i += 1
            while (
                allLines[i].startswith('#include "')
                and allLines[i] < '#include "decompress.h"\n'
            ):
                i += 1
