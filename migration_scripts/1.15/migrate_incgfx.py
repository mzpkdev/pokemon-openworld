...
def expand_variables(definition, variables):
    return re.sub(
        r"\$[({](\w+)[)}]", lambda m: variables.get(m.group(1), m.group(0)), definition
    )
...
def parse_graphics_rules():
    ...
    for generic_target, value in generic_targets.items():
        ...
        generic_targets_.append(
            (r"^" + re.escape(prefix) + r"(.*)(" + re.escape(suffix) + r")$", value)
        )
...
def try_convert_incbin(size, c_source, pos, targets, generic_targets):
    ...
    if rule := lookup_rule(target, targets, generic_targets):
        ...
        if arguments:
            return (
                f'INCGFX_{size}("{source}", "{target_ext}", "{arguments}")'
                + c_source[pos:]
            )
        else:
...
def convert_incbins(path, targets, generic_targets):
    ...
    while True:
        ...
        if not m:
            break
        pos = m.start()
        rest = try_convert_incbin(
            m.group(1), c_source[pos:], m.end() - pos, targets, generic_targets
        )
        if rest:
