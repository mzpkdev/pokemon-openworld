...
def stack_sprite(name, path):
    joinp = os.path.join
    frames = [
        joinp(path, "down", name),
        joinp(path, "down", "frame2", name),
        joinp(path, "up", name),
        joinp(path, "up", "frame2", name),
        joinp(path, "left", name),
        joinp(path, "left", "frame2", name),
    ]
...
def apply_palette(
    palette_file, input_file, output_file
):  # Apply one file's palette to another
    ...
    with open(output_file, "wb") as f:
        ...
        for row in rows:
            new_rows.append(
                [closest_color(src_palette[c], target_palette) if c else 0 for c in row]
            )
...
def paletteify(path, output_path=None):
    ...
    # Strip alpha color
    subprocess.run(
        ["convert", path, "-background", color, "-alpha", "remove", output_path],
        check=True,
    )
    apply_palette(
        joinp(PKMN_GRAPHICS, species, "anim_front.png"), output_path, output_path
    )
    ...
    if args:
        ...
        for path in sorted(glob("overworld/*.png")):
            ...
            output_path = os.path.join(
                "graphics/object_events/pics/pokemon", f"{name}.png"
            )
            try:
