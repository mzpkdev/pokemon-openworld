...
if (-Not (Test-Path ".\pokemon-openworld.gba"))
{
    Write-Host "File 'pokemon-openworld.gba' not found."
    Write-Host $help
    exit
}
...
&$objdump -D -bbinary -marmv4t -Mforce-thumb --start-address="$start_str" --stop-address="$end_str" .\pokemon-openworld.gba > .\pokemon-openworld.dump
Invoke-Expression "$DiffTool .\baserom.dump .\pokemon-openworld.dump"
